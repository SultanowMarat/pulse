                    ...
                    <tr key={u.id} className="border-t border-sidebar-border/30">
                      ...
                      <td className="px-3 py-2 text-right">
                        <div className="flex flex-wrap items-center justify-end gap-2">
                          <button type="button" onClick={() => setEditUser(u)} className="h-8 px-3 rounded-compass bg-sidebar-hover text-white text-[12px] font-medium hover:brightness-110 transition whitespace-nowrap">
                            Изменить
                          </button>
                          <button
                            type="button"
                            ...
                            className="h-8 px-3 rounded-compass bg-danger/20 text-danger text-[12px] font-medium hover:brightness-110 transition whitespace-nowrap"
...
function UserEditorModal({
  ...
  const [generatingLoginKey, setGeneratingLoginKey] = useState(false);
  ...
  const validate = () => {
    ...
    if (email.trim() && !EMAIL_RE.test(email.trim().toLowerCase())) return 'Неверный формат email';
    return '';
  };
  ...
  const handleGenerateLoginKey = async () => {
    if (!initial?.id || generatingLoginKey) return;
    setGeneratingLoginKey(true);
    setError('');
    try {
      const res = await api.generateUserLoginKey(initial.id);
      setLoginKey(res.login_key || '');
      onNotify?.('Ключ входа сгенерирован');
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Не удалось сгенерировать ключ входа';
      setError(msg);
      onNotify?.(`Ошибка: ${msg}`);
    } finally {
      setGeneratingLoginKey(false);
    }
  };
  ...
  const handleCopyLoginKey = async () => {
    ...
    try {
      ...
      onNotify?.('Ключ скопирован');
    } catch {
      onNotify?.('Не удалось скопировать ключ');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 safe-area-padding">
      ...
      <div className="relative w-full max-w-[560px] max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-compass bg-sidebar border border-sidebar-border/50 p-4 space-y-3">
        ...
        <Field label="Email (опционально)" value={email} onChange={setEmail} placeholder="email@example.com" />
        ...
        {initial?.id && (
          <section className="rounded-compass border border-sidebar-border/40 p-3 space-y-2">
            <div className="flex items-center justify-between gap-2">
              <span className="text-[12px] text-sidebar-text">Ключ входа</span>
              <div className="flex items-center gap-2">
                {loginKey && (
                  <button
                    type="button"
                    onClick={handleCopyLoginKey}
                    className="h-8 px-3 rounded-compass bg-sidebar-hover text-white text-[12px] font-medium hover:brightness-110 transition whitespace-nowrap"
                  >
                    Скопировать
                  </button>
                )}
                <button
                  ...
                  disabled={generatingLoginKey}
                  className="h-8 px-3 rounded-compass bg-primary text-white text-[12px] font-semibold hover:brightness-110 transition disabled:opacity-60 whitespace-nowrap"
                >
                  {generatingLoginKey ? 'Генерация...' : 'Сгенерировать ключ'}
                </button>
              </div>
            </div>
            <input
              ...
              value={loginKey}
              readOnly
              placeholder="Ключ появится после генерации"
              className="w-full h-10 rounded-compass bg-sidebar border border-sidebar-border/40 px-3 text-[13px] text-white placeholder:text-sidebar-text/80 outline-none"
            />
            <p className="text-[12px] text-warning leading-snug">
              Сохраните ключ сразу после генерации. После повторной генерации старый ключ станет недействительным.
            </p>
            <p className="text-[12px] text-sidebar-text leading-snug">
              Ключ для входа без email. Доступен только для 3 попыток входа, после чего автоматически сбрасывается.
            </p>
          </section>
        )}
